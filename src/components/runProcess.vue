...
export default {
  ...
  mounted() {
    ...
    this.$bus.$on(this.$busEvents.drawerIsShow, (_type, batch_id, report_id) => {
      if (_type === 'process') {
        ...
        this.isShowReportPage = false // 初始化时，标识未打开测试报告页面
  ...
  methods: {
    ...
    getShowReportId(batch_id) {
      const that = this
      if (that.isShowReportPage === false) {
        this.reportShowIdUrl({ batch_id: batch_id }).then(response => {
          that.isShowReportPage = true // 一次运行，只打开一次测试报告（查询的时候可能会存在多个同时返回成功的情况）
    ...
    // async getTestProcess(batch_id) {
    //   const that = this
    //   that.res = undefined
    //   this.reportStatusUrl({
    //     batch_id: batch_id,
    ...
    async getReport(batch_id) {
      // 触发运行成功，每1.5秒查询一次，
      // 最后都没查出结果，则停止查询，提示用户去测试报告页查看
      // 已出结果，则停止查询，展示测试报告

      const runTimeoutCount = Number(this.$busEvents.runTimeout) * 1000 / 1500
      for (var i = 1; i < runTimeoutCount; i++) {
        if (this.processIsShow === true) {
          const reportStatusResponse = await this.reportStatusUrl({
            batch_id: batch_id, process: this.activeProcess, status: this.activeStatus
          })

          if (reportStatusResponse.status === 200) {
            this.activeProcess = reportStatusResponse.data.process
            this.activeStatus = reportStatusResponse.data.status

            if (this.reportId && this.activeProcess === 2) { // 执行中
              // 获取执行的用例
              const reportCaseListResponse = await this.reportCaseListUrl({ report_id: this.reportId })
              this.reportCaseDataList = reportCaseListResponse.data
              // that.getRunningReportCaseId(that.reportCaseDataList)  // 自动查最新的在执行的用例
              if (!this.reportCaseId) {
                this.reportCaseId = reportCaseListResponse.data[0].id
              }

              // 获取指定用例的执行步骤
              const reportStepListResponse = await this.reportStepListUrl({ report_case_id: this.reportCaseId })
              this.reportStepDataList = reportStepListResponse.data
            } else if (this.activeProcess === 3 && this.activeStatus === 2) {
              this.processIsShow = false // 关闭进度框
              this.getShowReportId(batch_id)
              break
            }
          } else {
            this.processIsShow = false // 关闭进度框
      ...
      if (this.processIsShow === true) {
        this.$notify(runTestTimeOutMessage(this))
        this.processIsShow = false // 关闭进度框
      }
    },
