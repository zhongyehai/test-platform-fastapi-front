...
} from '@/apis/appTest/caseSuite'
import { getConfigByCode } from '@/apis/config/config'
import { phoneList, serverList } from '@/apis/appTest/device'
...
export default {
  ...
  methods: {
    ...
    sendSuiteTreeIsDone(caseSuiteTree) {
      this.$bus.$emit(
        this.$busEvents.treeIsDone, 'caseSuite', JSON.parse(JSON.stringify(caseSuiteTree)), 'caseIndex'
      )
    },
    ...
    getSuiteList(projectId) {
      ...
      this.currentParent = {}
      this.caseSuiteTreeUrl({ page_num: 1, page_size: 99999, project_id: projectId }).then(response => {
    ...
    // 添加用例
    addCase(node, data) {
      this.$bus.$emit(this.$busEvents.drawerIsShow, 'caseInfo', 'add', null, 'caseIndex')
    },
