...
import { projectList as webUiProjectList } from '@/apis/uiTest/project'
import { caseSuiteList as webUiCaseSuiteTree } from '@/apis/uiTest/caseSuite'
import { caseList as webUiCaseList, copyCaseStep as webUiCopyCaseStep } from '@/apis/uiTest/case'
import { stepCopy as webUiStepCopy, stepList as webUiStepList } from '@/apis/uiTest/step'

import { projectList as appUiProjectList } from '@/apis/appTest/project'
import { caseSuiteList as appUiCaseSuiteTree } from '@/apis/appTest/caseSuite'
import { caseList as appUiCaseList, copyCaseStep as appUiCopyCaseStep } from '@/apis/appTest/case'
import { stepCopy as appUiStepCopy, stepList as appUiStepList } from '@/apis/appTest/step'
  ...
  methods: {
    ...
    getStep(row, treeNode, resolve) {
      if (this.expands.indexOf(row.row_id) === -1) {
        ...
        this.stepListUrl({ case_id: request_id, page_num: 1, page_size: 99999 }).then(response => {
    ...
    // 获取服务列表
    getProjectList() {
      this.projectListUrl({ page_num: 1, page_size: 99999 }).then(response => {
    ...
    getCaseList(caseSuiteList) {
      ...
      this.caseListUrl({
        page_num: this.quotePageNum,
        page_size: this.quotePageSize,
        suite_id: this.currentSetId,
        status: 1,
        has_step: true
      }).then(response => {
