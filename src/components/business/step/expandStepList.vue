...
import { caseFrom as webUiCaseFrom, getCase as webUiGetCase } from '@/apis/uiTest/case'

import {
  putStepIsRun as appUiPutStepIsRun,
  stepList as appUiStepList
} from '@/apis/appTest/step'
import { caseFrom as appUiCaseFrom, getCase as appUiGetCase } from '@/apis/appTest/case'
  ...
  methods: {
    ...
    changeExpandStatus(row, expandedRows) {
      ...
      if (expandIdIndex === -1) {
        ...
        this.stepListUrl({ case_id: row.quote_case, page_num: 1, page_size: 99999 }).then(response => {
          this.sendBus('getExpandData')
    ...
    getStepList(case_id) {
      this.tableLoadingIsShow = true
      this.stepListUrl({ 'case_id': case_id || this.currentCaseId, page_num: 1, page_size: 99999 }).then(response => {
        this.tableLoadingIsShow = false
