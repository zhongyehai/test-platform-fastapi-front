...
import { getPage as appUiGetPage } from '@/apis/appTest/page'
import {
...
} from '@/apis/appTest/element'
import {
  postStep as appUiPostStep,
  putStep as appUiPutStep
} from '@/apis/appTest/step'

import { getPage as webUiGetPage } from '@/apis/uiTest/page'
import {
  ...
  mounted() {
    ...
    if (this.dataType !== 'api') {
      if (!this.$busEvents.data.executeTypeList || this.$busEvents.data.executeTypeList.length === 0) {
        getConfigByCode({ code: 'ui_action_mapping_list' }).then(response => {
