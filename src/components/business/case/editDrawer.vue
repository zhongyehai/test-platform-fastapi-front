...
} from '@/apis/uiTest/case'
import { getCaseSuite as webUiGetCaseSuite } from '@/apis/uiTest/caseSuite'
...
} from '@/apis/appTest/case'
import { getCaseSuite as appUiGetCaseSuite } from '@/apis/appTest/caseSuite'
...
export default {
  ...
  mounted() {
    ...
    // 从后端获取数据类型映射
    if (!this.$busEvents.data.dataTypeMappingList || this.$busEvents.data.dataTypeMappingList.length === 0) {
    ...
    if (this.dataType !== 'api') {
      // 从后端获取UI测试数据提取方式映射
      if (!this.$busEvents.data.uiTestExtractMappingList || this.$busEvents.data.uiTestExtractMappingList.length === 0) {
        getConfigByCode({ code: 'ui_extract_mapping_list' }).then(response => {
    ...
    // 从后端获取跳过方式映射
    if (!this.$busEvents.data.skipIfTypeMappingList || this.$busEvents.data.skipIfTypeMappingList.length === 0) {
    ...
    // 从后端获取跳过条件映射，用例
    if (!this.$busEvents.data.caseSkipIfDataSourceMapping || this.$busEvents.data.caseSkipIfDataSourceMapping.length === 0) {
    ...
    // 从后端获取跳过条件映射，步骤
    if (!this.$busEvents.data.stepSkipIfDataSourceMapping || this.$busEvents.data.stepSkipIfDataSourceMapping.length === 0) {
    ...
    if (this.dataType === 'appUi') {
      // 从后端获取键盘映射
      if (!this.$busEvents.data.keyboardKeyCodeList || this.$busEvents.data.keyboardKeyCodeList.length === 0) {
        getConfigByCode({ code: 'app_key_board_code' }).then(response => {
    ...
    // 从后端获取响应对象数据源映射
    if (!this.$busEvents.data.responseDataSourceMappingList || this.$busEvents.data.responseDataSourceMappingList.length === 0) {
    ...
    // 从后端获取断言数方式映射
    if (!this.$busEvents.data.apiTestAssertMappingList || this.$busEvents.data.apiTestAssertMappingList.length === 0) {
    ...
    if (this.dataType === 'webUi') {
      if (!this.$busEvents.data.keyboardKeyCodeList || this.$busEvents.data.keyboardKeyCodeList.length === 0) {
        getConfigByCode({ code: 'ui_key_board_code' }).then(response => {
          this.$busEvents.data.keyboardKeyCodeList = response.data
        })
      }
    }
