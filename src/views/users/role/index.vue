<template>
  <div class="app-container">
    ...
    <el-table
      ...
      style="width: 100%;"
    >
      <el-table-column :label="'序号'" prop="id" align="center" min-width="8%" size="mini">
        <template slot-scope="scope">
          <span> {{ (listQuery.page_num - 1) * listQuery.page_size + scope.$index + 1 }} </span>
        </template>
      </el-table-column>

      <el-table-column :label="'角色名'" prop="name" align="left" min-width="15%" size="mini">
        <template slot-scope="scope">
          <span>{{ scope.row.name }}</span>
        </template>
      </el-table-column>

      <el-table-column show-overflow-tooltip :label="'备注'" prop="desc" align="left" min-width="33%" size="mini">
        <template slot-scope="scope">
          <span>{{ scope.row.desc }}</span>
        </template>
      </el-table-column>

      <el-table-column :label="'创建时间'" prop="create_time" align="left" min-width="18%" size="mini">
        <template slot-scope="scope">
          <span>{{ scope.row.create_time }}</span>
        </template>
      </el-table-column>

      <el-table-column :label="'修改时间'" prop="update_time" align="left" min-width="18%" size="mini">
        <template slot-scope="scope">
...
export default {
  ...
  mounted() {
    permissionList({ page_num: 1, page_size: 99999 }).then(response => {
