<template>
  <div class="app-container">
    ...
    <el-table
      ...
      style="width: 100%;"
    >
      <el-table-column :label="'序号'" prop="id" align="left" min-width="6%" size="mini">
        <template slot-scope="scope">
          <span> {{ (listQuery.page_num - 1) * listQuery.page_size + scope.$index + 1 }} </span>
        </template>
      </el-table-column>

      <el-table-column :label="'权限类型'" prop="source_addr" align="left" min-width="10%" size="mini">
        <template slot-scope="scope">
          <span>{{ source_type_dict[scope.row.source_type] }}</span>
        </template>
      </el-table-column>

      <el-table-column :label="'权限名'" prop="name" align="left" min-width="25%" size="mini">
        <template slot-scope="scope">
          <span>{{ scope.row.name }}</span>
        </template>
      </el-table-column>

      <el-table-column :label="'权限地址'" prop="source_addr" align="left" min-width="25%" size="mini">
        <template slot-scope="scope">
          <span>{{ scope.row.source_addr }}</span>
        </template>
      </el-table-column>

      <el-table-column :label="'备注'" prop="desc" align="left" min-width="10%" size="mini">
        <template slot-scope="scope">
