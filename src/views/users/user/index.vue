  ...
  <div class="app-container">
    ...
    <el-form label-width="65px" :inline="true">
      ...
      <el-form-item :label="'状态：'" size="mini">
        <el-select
          ...
          class="filter-item"
        >
          <el-option v-for="status in status_list" :key="status.label" :label="status.label" :value="status.value" />
        </el-select>
      </el-form-item>
    ...
    >
      ...
      <el-table-column align="center" prop="status" label="启用状态" min-width="15%">
        <template slot-scope="scope">
          <el-switch
            v-model="scope.row.status === 'enable'"
...
export default {
  ...
  data() {
    return {
      ...
      user_list: [], // 用户列表
      roleList: [], // 角色列表
      status_list: [{ value: 'enable', label: '启用' }, { value: 'disable', label: '冻结' }], // 状态列表
      business_list: [], // 业务线列表
      tableKey: 0, // 服务数据表格起始
      total: 0, // 服务数据表格总条数
  ...
  mounted() {
    ...
    if (this.$busEvents.data.businessList.length < 1) {
      businessList({ page_num: 1, page_size: 99999 }).then(response => {
  ...
  methods: {
    ...
    // 初始化用户状态
    initStatus() {
      this.status_list = [{ value: 'enable', label: '启用' }, { value: 'disable', label: '冻结' }]
    },
    ...
    // 获取角色列表
    getRoleList() {
      roleList({ page_num: 1, page_size: 99999 }).then(response => {
