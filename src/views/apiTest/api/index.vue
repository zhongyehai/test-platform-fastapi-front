          ...
          >
            <template slot-scope="scope">
              <div
                ...
                :style="{
                  'backgroundColor': scope.row.status === 'disable' ? '#ebebeb' : '',
                  'textDecoration': scope.row.status === 'disable' ? 'line-through' : ''
                }"
              >
                <span
          ...
          <el-table-column align="center" min-width="10%">
            <template slot="header">
              ...
              <el-tooltip class="item" effect="dark" placement="top-start">
                <div slot="content">
                  <div>标识接口是否可用，对应swagger的是否被废弃</div>
                </div>
                <span><i style="color: #409EFF" class="el-icon-question" /></span>
              </el-tooltip>
            </template>
...
export default {
  ...
  methods: {
    ...
    runApis(runConf) {
      ...
      runApi({
        project_id: this.currentApi.project_id,
        api_list: [this.currentApi.id],
