  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column align="center" prop="status" label="启用状态" min-width="15%">
        <template slot-scope="scope">
          <el-switch v-model="scope.row.task_code !== undefined" @change="changStatus(scope.row)" />
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getCopyData() {
      return JSON.stringify(this.showDetail, null, 4)
    },
    ...
    showRunJobLog(row) {
      ...
      jobRunLog({ func_name: this.currentFuncName, page_num: this.page_num, page_size: this.page_size }).then(response => {
    ...
    runJob(row) {
      this.$set(row, 'runPopoverIsShow', false)
      jobRun({ func: row.func_name }).then(response => {
    ...
    changStatus(row) {
      ...
      if (row.task_code) { // 关闭任务
        deleteJob({ task_code: row.task_code }).then(response => {
          this.listLoading = false
          if (this.showMessage(this, response)) {
            this.getJobFuncList()
          }
