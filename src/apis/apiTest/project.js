...
const currentBaseDir = baseDirApiTest + '/project'
const currentEnvDir = currentBaseDir + '/env'
...
export function getProjectEnv(data) {
  return request({ url: currentEnvDir, method: 'post', data: data })
}

// 修改改服务的环境信息
export function putProjectEnv(data) {
  return request({ url: currentEnvDir, method: 'put', data: data })
}
...
export function projectEnvSynchronization(data) {
  return request({ url: currentEnvDir + '/synchronization', method: 'put', data: data })
}
